  ...
  <div v-if="(activeModel !== undefined)">
    ...
    <div class="card mb-3">
      ...
      <div class="card-body">
        ...
        <dl class="row" :class="{ 'loading': getActiveModelData('date_created') === '' }">
          ...
          <dd class="col-sm-8"><span v-if="getActiveModelData('date_created') === ''">-</span><span v-else>{{ shareLevelText }}</span></dd>

          <dt class="col-sm-4 text-right">Created</dt>
          <dd class="col-sm-8"><span v-if="getActiveModelData('date_created') === ''">-</span><span v-else>{{ dateCreatedText }}</span></dd>

          <dt class="col-sm-4 text-right">Run environment</dt>
          <dd class="col-sm-8"><span v-if="getActiveModelData('date_created') === ''">-</span><span v-else>Amazon</span></dd>
        ...
        <dl class="row">
          ...
          <dd class="col-sm-9 m-auto">
            ...
            <template v-else>
              <a class="btn btn-outline-secondary btn-sm mr-2" :href="getActiveModelData('fileurl')" target="_blank" title="A link to the file server hosting all output files.">
                File Server
              </a>
              <a class="btn btn-outline-secondary btn-sm" :href="`/thredds/catalog/files/${getActiveModelData('suid')}/simulation/catalog.html`" target="_blank" title="A link to the THREDDS server hosting Delft3D NetCDF output, which allows querying via OPeNDAP.">
                THREDDS Data Server
              </a>
            </template>
          </dd>
    ...
    <div class="card mb-3">
      ...
      <div id="simulation-controls-collapse" class="collapse card-body">
        <div class="card-body">
          ...
          <button type="button" class="btn btn-labeled btn-spaced-right" id="simulation-control-start" @click="startModel" :class="[(isReadOnly || !isIdle) ? 'disabled' : 'btn-info']" :disabled="(isReadOnly || !isIdle)">
            <span class="btn-label"><i class="fa fa-fw fa-play" aria-hidden="true"></i></span>
            Start
          </button>&nbsp;
          <button type="button" @click="stopModel" class="btn btn-labeled btn-spaced-right" id="simulation-control-stop" :class="[(isReadOnly || (!isRunning && !isQueued)) ? 'disabled' : 'btn-success']" :disabled="(isReadOnly || (!isRunning && !isQueued))">
            <span class="btn-label"><i class="fa fa-fw fa-stop" aria-hidden="true"></i></span>
            Stop
          </button>&nbsp;
          <button type="button" @click="updateModelBy = {name: 'reset', modelContainer: activeModel}" class="btn btn-labeled btn-spaced-right" id="simulation-control-stop" :class="[(isReadOnly || !isFinished) ? 'disabled' : 'btn-warning']" :disabled="(isReadOnly || !isFinished)">
            <span class="btn-label"><i class="fa fa-fw fa-fast-backward" aria-hidden="true"></i></span>
            Reset
          </button>&nbsp;
          <div class="btn-group">
            <button class="btn dropdown-toggle" :class="[(isReadOnly || !isFinished || outdated == false) ? 'disabled' : 'btn-warning']" data-toggle="dropdown" href="#" :disabled="(isReadOnly || !isFinished || outdated == false)"
              :data-original-title="(outdated == false) ? 'no updates available' : getActiveModelData('outdated_workflow')">
              <span class="btn-label">
                <i class="fa fa-fw fa-level-up" aria-hidden="true"></i></span>
              Update
            ...
            <ul class="dropdown-menu" :disabled="(isReadOnly || !isFinished || outdated == false)">
              <li v-for="(entrypoint, index) in getEntrypoints" :key="index">
                <a @click="updateModelBy = {name: 'redo', entrypoint: entrypoint, modelContainer: activeModel}" href="#">
                  {{entrypoint}}
                </a>
              </li>
            </ul>
    ...
    <div class="card mb-3">
      <div class="card-header" data-toggle="collapse" data-target="#share-controls-collapse">
        Share controls
      </div>
      <div class="collapse" id="share-controls-collapse">
        <div class="card-body">
          ...
          <button type="button" class="btn btn-labeled btn-outline-secondary mr-2" :disabled="!isFinished || sharedState.user.id != getActiveModelData('owner.id') || getActiveModelData('shared') == 'c' || getActiveModelData('shared') == 'w'" @click="updateModelBy = {name: 'publish', domain: 'company', modelContainer: activeModel}">
            <i class="fa fa-group" aria-hidden="true"></i> Share with Company
          </button>
          <button type="button" class="btn btn-labeled btn-outline-secondary" :disabled="!isFinished || sharedState.user.id != getActiveModelData('owner.id') || getActiveModelData('shared') == 'w'" @click="updateModelBy = {name: 'publish', domain: 'world', modelContainer: activeModel}">
    ...
    <div class="card mb-3">
      <div class="card-header" data-toggle="collapse" data-target="#viewer-collapse">
        3D Viewer
      </div>
      <div class="collapse" id="viewer-collapse">
        <div class="card-body">
          <viewer-3d :model="activeModel.data" :activated="viewerActive"></viewer-3d>
        </div>
      </div>
    </div>

    <div class="card mb-3">
      <div class="card-header" data-toggle="collapse" data-target="#generated-images-collapse">
        Generated images
      </div>
    ...
    <div class="card mb-3">
      <div class="card-header" data-toggle="collapse" data-target="#input-parameters-collapse">
    ...
    <div class="card mb-3">
      <div class="card-header" data-toggle="collapse" data-target="#post-processing-collapse">
    ...
    <confirm-dialog v-if="updateModelBy.name" :confirm-button-title="capitalizeFirst(updateModelBy.name)" :dialogId="`confirm-${updateModelBy.name}`" @confirm="confirm">
      <template slot="title">
        {{capitalizeFirst(updateModelBy.name)}} model '{{ getActiveModelData('name') }}'?
      </template>
      <template slot="body">
        <p>Are you sure you want to {{updateModelBy.name}} this model? This action cannot be undone.</p>
      </template>
    </confirm-dialog>
  </div>
